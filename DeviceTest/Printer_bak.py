...
class Printer:
    ...
    def printRaw(self, title, car_info, H_M_S, cost, real_cost, change, voucher, type, in_time, out_time, codebar):
        ...
        if type == "现金支付":
            toPrintTexts = [
                ...
                "   应收金额  %.2f元" % (cost) + ' ' * (len(car_info) - len(str(cost)) + 4) + '实收金额  ' + '%.2f元\0' % (
                    real_cost),
                "   找零金额  %.2f元" % (change) + ' ' * (len(car_info) - len(str(change)) + 4) + "优惠券   " + '%s' % voucher,
                "   缴费方式  %s\0" % (type),
                "   入场时间  %s\0" % in_time,
                "   出场时间  %s\0" % out_time
            ]
      ...
      #  self.generateQBar(codebar)
        ...
        #
        # 切
        rtn = self.dll.CeTCut(hComContext, byref(dwSysErr))
        print("cut:", rtn, dwSysErr.value)
        self.ui.emit(self.ui.PRINTSTATUS)
    ...
    def checkOutOfPaper(self):
        ...
        print("status:", rtn, dwStatus, dwSysErr.value)
        return dwStatus
            ...
            try:
                while self.isRunning:
                 ...
                 #   print("setTextMode:", rtn, dwSysErr.value)
                    ...
                    txtBytes = titleAll.encode('gbk')  # 编码成gbk
                ...
                #    print("center justy:", rtn, dwSysErr.value)
                    ...
                    # self.ui.emit(self.ui.PRINTSTATUS)
                    successNo += 1
                    self.ui.logUi.log("----------打印成功，次数："+str(successNo))
                    import random
...
if __name__ == '__main__':
    ...
    printer.printRaw('天津国际机场','津Q45689','3小时20分45秒',12.6,15,2.4,'无','线上支付','2019-3-24-10:30:45','2019-3-24-1:30:51','http://www.baidu.com')
